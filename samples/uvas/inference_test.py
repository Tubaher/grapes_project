		...
		for frameCount in range(totalFrames): 
			...
			if success:
				...
				# Dataset division into two classes 
                    ...
                    g_bbox_xywh_red = bbox_xywh_red
                    g_cls_conf_red = results['red_scores']


                    if (tracking):
                        if(g_bbox_xywh_red is not [] and len(g_bbox_xywh_red)!=0):

                            #Put the prediction confidence in each bbox
                            for i in range(len(g_bbox_xywh_red)):
                                cv2.putText(splash, str(g_cls_conf_red[i]), (int(g_bbox_xywh_red[i][0]), int(g_bbox_xywh_red[i][1])), cv2.FONT_HERSHEY_PLAIN, 1, [255, 255, 255], 1)
                            

						    outputs_red = deepsort_red.update(g_bbox_xywh_red, g_cls_conf_red, splash)


                            if len(outputs_red) > 0:
                                # Draw the tracker box # output red row [x1,y1,x2,y2,id_racimo]
                                new_bbox_xyxy_red = outputs_red[:, :4]
                                identities_red = outputs_red[:, -1] 
                                splash = draw_boxes(splash, new_bbox_xyxy_red, identities_red, class_label="Uva")

                                #Get the racimo ids if it is not include in the dict
                                #with its respective frame Count and x_center
                                for k in range(output_red.shape[0]):
                                    id_racimo = identities_red
