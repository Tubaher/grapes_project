...
def map_cuartel(pickles_dir,ax, DATOS):
	...
	frames_cuartel = {}
	df_cuartel = DATOS[(DATOS["CAMPO_ID"]==campo) & (DATOS["CUARTEL_ID"]==cuartel)]
	...
	pts_diagonal = split(polygon_diagonal[0],polygon_diagonal[1],n_hileras+1)
	...
	for hilera_idx in range(hilera_min,hilera_max+1):
		hilera_pts[hilera_idx] = (start_points[hilera_idx-hilera_min],end_points[hilera_idx-hilera_min])
	...
	for key,value in frames_cuartel.items():
		...
		if am_exist and pm_exist:
			...
			counts_am, segments = count_intervals(am_frames,\
				hilera_coord,am_frame_lim,DELTA_H)
			counts_pm , _ = count_intervals(pm_frames,\
				hilera_coord,pm_frame_lim,DELTA_H)
			...
			if args.area is not None:
				am_areas = df_cuartel[(df_cuartel["HILERA_ID"]==str(key))&(df_cuartel["AM/PM"]=="1")].RACIMO_AREA.to_numpy()
				pm_areas = df_cuartel[(df_cuartel["HILERA_ID"]==str(key))&(df_cuartel["AM/PM"]=="2")].RACIMO_AREA.to_numpy()

				count_area_am = np.zeros(len(counts_am))
				count_skp_am = 0
				for count_indx in range(len(counts_am)):
					count = counts_am[count_indx]
			...
			if args.area:
				...
				counts_total = counts_total/5000
			else:
