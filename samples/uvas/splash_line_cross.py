...
# or left. If the first grape bunches appear of the left part of the video
VIDEO_DIRECTION = 'right'
...
def detect_and_color_splash(model):
    ...
    if args.video:
            ...
            if success:
                ...
                for i, result in enumerate(results):
                    ...
                    if(detections_red is not [] and len(detections_red) != 0):
                        ...
                        bbox_xywh_red[:, 0] = (
                            detections_red[:, 1] + detections_red[:, 3])/2  # x_center
                        bbox_xywh_red[:, 1] = (
                            detections_red[:, 0] + detections_red[:, 2])/2  # y_center
                        bbox_xywh_red[:, 2] = (
                            detections_red[:, 3] - detections_red[:, 1])  # width
                        bbox_xywh_red[:, 3] = (
                            detections_red[:, 2] - detections_red[:, 0])  # height
                        ...
                        if len(outputs_red) > 0:
                            ...
                            splash = draw_boxes(
                                splash, bbox_xyxy_red, identities_red, class_label="Uva")
                            ...
                            for identity in identities_red:
                                area = deepsort_red.tracker.get_area_by_id(
                                    identity)
                                if area >= 0:
        ...
        print("[INFO] Total Proccessing Duration Time: {}".format(
